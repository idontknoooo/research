#include "header.h"
// C: a line of information stored in vector which was originally coming from Excel
// Get first position of C (also "name" of C)
string get_name ( const vector<string> &C ) {
    return C[0];
}
// Given a vector C, return true if C contains army backgrounds
...
// Given a vector C, return true if C contains political backgrounds
...
// Given a vector C, return true if C contains teacher backgrounds
...
// Given a vector C, return party information of C
...
// Given a vector C, return works information of C
    ...
    for ( int i = 0; i < Size(C); i++ ) {
        ...
        for ( int j = 0; j < Size(Begins); j++ )
            if ( C[i].find ( Begins[j] ) != string::npos ) {
                ...
                if ( location_p > t ) location_p = t; // location_p is index of work location in C
            }
...
// Given a vector C, return information regards to "school" and "teacher" info
...
// Given a vector C, return the info of school
